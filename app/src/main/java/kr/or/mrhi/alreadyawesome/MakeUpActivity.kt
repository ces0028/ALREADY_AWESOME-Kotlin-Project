...
import kr.or.mrhi.alreadyawesome.databinding.ActivityMakeupBinding
import kr.or.mrhi.alreadyawesome.databinding.DialogSortShopBinding

class MakeUpActivity : AppCompatActivity() {
    companion object {
        const val MAKEUP_KO = "메이크업업"
        const val MAKEUP_EN = "MAKEUP"
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        shopListAdapter = ShopListAdapter(applicationContext, MAKEUP_KO, shopList)
        binding.rvMakeUp.layoutManager = LinearLayoutManager(this)
        binding.rvMakeUp.adapter = shopListAdapter

        // 정렬기준 버튼을 클릭하면 다이얼로그 창이 출력
        binding.btnMakeUpSort.setOnClickListener {
            // 어느 화면을 출력할 건지 선택
        ...
        if (deleteFlag) {
            ...
            shopDAO.selectShop()?.addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    Log.d("kr.or.mrhi", "MakeUpActivity selectShop() onDataChange")
                }

                override fun onCancelled(error: DatabaseError) {
                    Log.d("kr.or.mrhi", "MakeUpActivity selectShop() onCancelled")
                }
            })
        }
    }
    ...
    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        ...
        searchView.setOnQueryTextListener(object: SearchView.OnQueryTextListener{
            ...
            override fun onQueryTextChange(query: String?): Boolean {
                val dbHelper = DBHelper(this@MakeUpActivity, MainActivity.DB_NAME, MainActivity.VERSION)
                // 검색어가 입력되지 않았을 때는 전체리스트를 출력
                // toolbar의 타이틀 역할을 하는 tvHair에 문자를 입력
                if (query.isNullOrBlank()) {
                    binding.tvMakeUp.text = MAKEUP_EN
                    shopList?.clear()
    ...
    // <price3 대신 replace.toInt를 한 이유>
    // price3가 문자열로 입력이 되어있어서 "70,000"과 "100,000" 등 5자리 숫자와 6자리 숫자를 올바르게 정렬하지 못 함
    // 그래서 문자열에 삽입되어있는 ','를 삭제하고, 그 값을 Int로 변환해서 정렬기준으로 사용하는 형식으로 진행함
    fun sortList(sortType: Int) {
        when (sortType) {
            1 -> shopList?.sortBy { it.price3.replace(",", "").toInt() }
            2 -> shopList?.sortByDescending { it.price3.replace(",", "").toInt() }
